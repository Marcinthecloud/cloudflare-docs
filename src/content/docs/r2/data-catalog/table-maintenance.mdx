...
description: Learn about how R2 Data Catalog handles table maintenance automatically for you
sidebar:
  order: 4
---
## What is table maintenance and why is it important?
...
### Configuring snapshot expiration
...
Both conditions must be met for a snapshot to be expired. This ensures you always retain recent snapshots even if they're older than the age threshold.
...
wrangler r2 bucket catalog snapshot-expiration enable my-bucket \
  --token $R2_CATALOG_TOKEN \
  --older-than-days 7 \
  --retain-last 10
...
wrangler r2 bucket catalog snapshot-expiration enable my-bucket my-namespace my-table \
  --token $R2_CATALOG_TOKEN \
  --older-than-days 2 \
  --retain-last 5
