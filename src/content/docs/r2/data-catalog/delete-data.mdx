...
import { FileTree, Tabs, TabItem, InlineBadge } from "~/components";
...
Apache Iceberg uses a layered metadata structure to manage table data efficiently.

### Components

Key Apache Iceberg components include:
...
Apache Iceberg reuses unchanged manifest files across snapshots to optimize storage and performance.
...
An Apache Iceberg table follows a specific structure. For example:
...
| **Maintenance needs** | Snapshot expiration                      | Snapshot expiration and compaction (`rewrite_data_files`) |
...
## Use PySpark to delete data from R2 Data Catalog
...
The following are basic examples to demonstrate proper deletion operations using PySpark with R2 Data Catalog. Each section shows a different category of operations.

#### DELETE operations

Delete operations mark files for cleanup but do not immediately remove them from R2 storage. Use maintenance operations to complete the cleanup.
...
Use the following examples to drop tables and namespaces in R2 Data Catalog. Use `PURGE` only when you want to permanently delete data files from R2.
...
def drop_examples(spark):
    ...
    # DANGER: Permanently deletes all data files from R2
...
:::note
R2 Data Catalog can [automatically run maintenance operations](/r2/data-catalog/table-maintenance/) for you. You only need to run maintenance operations manually for custom cleanup operations.
:::

#### Configure Spark

Use the following example to configure a Spark session for R2 Data Catalog with PySpark. Replace the placeholder values with your account details and store secrets securely.
...
Use the following example to perform a full operation that sets up Spark, deletes data, and performs cleanup.
