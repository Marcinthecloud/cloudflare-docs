...
Snapshot expiration automatically removes old table snapshots to reduce metadata bloat and storage costs. For more information about snapshot expiration and why it's valuable, refer to [Table maintenance](/r2/data-catalog/table-maintenance/).
...
Disabling snapshot expiration will prevent the process from running for all tables (catalog level) or a specific table (table level). You can re-enable it at any time.
