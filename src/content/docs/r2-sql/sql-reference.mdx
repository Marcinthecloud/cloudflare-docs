...
-- Invalid: No aliases
SELECT department, COUNT(*) AS total FROM sales_data GROUP BY department

-- Invalid: COUNT column name
SELECT COUNT(department) FROM sales_data

-- Invalid: SUM on integer
...
FROM table_name
GROUP BY column_list
HAVING COUNT(*) comparison_operator value
```
